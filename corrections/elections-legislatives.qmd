...
ggplot(legislatives_sf) +
  geom_histogram(aes(x = percent_abstentions), bins = 100)
```
...
On choisit donc une discrétisation capable de mettre en évidence cette longue "queue" de nos données, *head-tails breaks* :

```{r}
headtails = classInt::classIntervals(
...
ggplot(legislatives_sf,
  ...
  geom_sf(color = NA) +
  scale_fill_viridis_b(breaks = headtails$brks,
                       labels = round) +
